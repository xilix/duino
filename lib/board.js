...
var Board = function (options) {
  ...
  var self = this;
  if (options.device === "undefined") { otions.device = "usb"; }
 ...
 */
Board.prototype.detect = function (callback, device) {
  this.log('info', 'attempting to find Arduino board');
  var self = this;
  child.exec('ls /dev | grep '+device, function(err, stdout, stderr){
    ...
    while ( usb.length ) {
      ...
      if (possible.slice(0, 2) !== 'cu') {
        ...
        if (!err) {
          if (temp.port === undefined && temp.path !== undefined) { 
            temp.port = temp.path; 
          }
          found = temp;
